	...
	public String AddCriminal(Criminal_Info criminalInfo)
	{
		...
		try (Connection conn = DBUtil.provideConnection();)
		{
			PreparedStatement ps = conn.prepareStatement(
					"insert into criminal (Cname, Age, Gender, Address, Face_Id, Crime_Area, Crime_Name) "
							+ "values (?,?,?,?,?,?,?)");
	...
	public List<Criminal_Info> ViewCriminal() throws CriminalExcetion
	{
		...
		try (Connection conn = DBUtil.provideConnection();)
		{
			...
			while (rs.next())
			{
				...
				Criminal_Info cri = new Criminal_Info(id, n, a, g, ad, fi, area, cn);
		...
		if (Criminals.size() == 0) throw new CriminalExcetion("No Criminal found..");
	...
	public Criminal_Info getcriminalByCId(int Cid) throws CriminalExcetion
	{
		...
		try (Connection conn = DBUtil.provideConnection())
		{
			...
			if (rs.next())
			{
				...
				criminal = new Criminal_Info(id, n, a, g, ad, fi, area, cn);

			}
			else throw new CriminalExcetion("Criminal does not exist with ID " + Cid);
